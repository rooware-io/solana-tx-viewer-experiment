import { Cluster, TransactionError } from "@solana/web3.js";
//import { Cluster } from "providers/cluster";
//import { getProgramName } from "utils/tx";
//import { getTransactionInstructionError } from "utils/program-err";

function getProgramName(address: string, cluster: Cluster) {
...
export function parseProgramLogs(
  ...
  innerInstructionsStackHeights: number[];
} {
  let depth = 0;
  let prettyLogs: InstructionLogs[] = [];
  let innerInstructionsStackHeights: number[] = [];
    ...
    if (log.startsWith("Program log:")) {
      ...
      if (matches.length > 0) {
        ...
        if (depth === 0) {
          ...
          innerInstructionsStackHeights.push(depth);
        }
  ...
  return { instructionLogs: prettyLogs, innerInstructionsStackHeights };
}
